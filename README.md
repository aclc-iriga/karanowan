...
Tabulation System for Karanowan Fish-tival 2024
...
3. Clone or download this repository to your XAMPP **htdocs** folder.
   The final path should be `path_to/xampp/htdocs/karanowan`.
...
5. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `karanowan` and import [karanowan.sql](karanowan.sql) into it.

6. Open the terminal and navigate to the project directory **karanowan**.
...
9. Open your web browser and access <http://localhost:5193/karanowan/> to view the application.
...
2. Inside [phpMyAdmin](http://localhost/phpmyadmin),
   create a MySQL database named `test-karanowan` and import [karanowan.sql](karanowan.sql) into it.
...
2. Access the application by visiting `http://[host_name]/karanowan`,
   where `host_name` is the **IP address** or **host name** of the server in the network.
   For example:
     - <http://localhost/karanowan>
     - <http://192.168.1.99/karanowan>


#### Admin Dashboard
Access the **admin dashboard** at <http://localhost/karanowan/app>.
